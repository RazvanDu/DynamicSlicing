...
output_file="/storage/paulclotan/SmartSliceGPT/experiments/output-100.txt"
...
for i in {0..33}; do
    ...
    echo -e "Running with --slice-layer $i --slice-dimension 100 --add-dimension True" >> "$output_file"
    ...
    eval $base_command --slice-layer $i --slice-dimension 100 --add-dimension True >> "$output_file"

    sleep 2
    # Echo a header for the iteration with False
    echo -e "\nRunning with --slice-layer $i --slice-dimension 100 --add-dimension False" >> "$output_file"
    ...
    eval $base_command --slice-layer $i --slice-dimension 100 >> "$output_file"
    sleep 2
