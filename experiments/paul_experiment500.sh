...
output_file="/storage/paulclotan/SmartSliceGPT/experiments/output-500-2.txt"
...
eval $base_command --slice-layer 25 --slice-dimension 0 --add-dimension True >> "$output_file"
sleep 2

# Loop 32 times for each slice layer
for i in {18..33}; do
    # Echo a header for the iteration with True
    echo -e "Running with --slice-layer $i --slice-dimension 500 --add-dimension True" >> "$output_file"
    ...
    eval $base_command --slice-layer $i --slice-dimension 500 --add-dimension True >> "$output_file"

    sleep 2
    # Echo a header for the iteration with False
    echo -e "\nRunning with --slice-layer $i --slice-dimension 500 --add-dimension False" >> "$output_file"
    ...
    eval $base_command --slice-layer $i --slice-dimension 500 >> "$output_file"
    sleep 2
