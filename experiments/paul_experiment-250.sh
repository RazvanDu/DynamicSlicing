...
base_command="python3.11 run_slicegpt_perplexity.py --model microsoft/phi-2 --save-dir /storage/paulclotan/SmartSliceGPT/save_work2 --sparsity 0.25 --no-wandb --device cuda:3 --hf-token hf_ucILpakjSHaCoAHxZkBwPQLGdGorpzhcqvclear"

# Define the output file
output_file="/storage/paulclotan/SmartSliceGPT/experiments/output-250.txt"
...
for i in {0..33}; do
    ...
    echo -e "Running with --slice-layer $i --slice-dimension 250 --add-dimension True" >> "$output_file"
    ...
    eval $base_command --slice-layer $i --slice-dimension 250 --add-dimension True >> "$output_file"

    sleep 2
    # Echo a header for the iteration with False
    echo -e "\nRunning with --slice-layer $i --slice-dimension 250 --add-dimension False" >> "$output_file"
    ...
    eval $base_command --slice-layer $i --slice-dimension 250 >> "$output_file"
    sleep 2
